...
public class EbookAdapter extends ArrayAdapter<EbookData>{
	private Context context;
	private ArrayList<EbookData> arr_data;
	...
	public EbookAdapter(Context context, int resource,
			ArrayList<EbookData> arrData,int widthImg,int heightImg) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (arr_data != null) {
			holder.txtName.setText(arr_data.get(position).getName());
			LinearLayout.LayoutParams relative = new LinearLayout.LayoutParams(
					widthImg, heightImg);
			holder.img.setLayoutParams(relative);
			imageLoader.displayImage(arr_data.get(position).getImgs()
					, holder.img, displayImageOptions);
			if (!arr_data.get(position).getColors().equals("")) {
				holder.lnBg.setBackgroundColor(Color.parseColor(arr_data.get(position).getColors()));
			}else {
