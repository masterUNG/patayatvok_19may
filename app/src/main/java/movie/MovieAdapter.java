...
public class MovieAdapter extends ArrayAdapter<MovieData>{
	...
	private ArrayList<MovieData> arr_data;
	...
	public MovieAdapter(Context context, int resource,
			ArrayList<MovieData> arrData,int widthImg,int heightImg) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (arr_data != null) {
			holder.txtName.setText(arr_data.get(position).getMovie_name());
			LinearLayout.LayoutParams relative = new LinearLayout.LayoutParams(
					widthImg, heightImg);
			holder.img.setLayoutParams(relative);
			imageLoader.displayImage(arr_data.get(position).getMovie_img()
					, holder.img, displayImageOptions);
			if (!arr_data.get(position).getMovie_color().equals("")) {
				holder.lnBg.setBackgroundColor(Color.parseColor(arr_data.get(position).getMovie_color()));
			}else {
